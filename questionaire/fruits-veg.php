...
<head>
  ...
  <title>استبيان تقييم خطر الإصابة بالسكري النوع الثاني - الخضار والفواكه</title>
  <style>
    @font-face {
...
<div class="page">
  ...
  <div class="card" role="main">
    <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; height: 100%;">
      <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['fruits-veg.php']) ? $_SESSION['survey_responses']['fruits-veg.php'] : 'no' ?>">

      <h1 class="title">استبيان تقييم خطر الإصابة بالسكري النوع الثاني</h1>

      <p class="question">هل تتناول الخضار أو الفواكه يومياً؟</p>

      <div class="toggle-row">
        <!-- لا — NO -->
        <button type="button" id="btn-no" class="tog-btn <?= (isset($_SESSION['survey_responses']['fruits-veg.php']) && $_SESSION['survey_responses']['fruits-veg.php'] == 'no') ? 'on' : ((!isset($_SESSION['survey_responses']['fruits-veg.php'])) ? 'on' : 'off') ?>" onclick="pick('no')" aria-label="لا">
          <span class="t-icon" id="icon-no"></span>
          لا
        </button>

        <!-- نعم — YES -->
        <button type="button" id="btn-yes" class="tog-btn <?= (isset($_SESSION['survey_responses']['fruits-veg.php']) && $_SESSION['survey_responses']['fruits-veg.php'] == 'yes') ? 'on' : 'off' ?>" onclick="pick('yes')" aria-label="نعم">
