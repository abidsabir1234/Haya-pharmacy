  ...
  <div class="card" role="main">
    <div class="card-body">
      <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; flex: 1;">
        <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['bmi-check-bp.php']) ? $_SESSION['survey_responses']['bmi-check-bp.php'] : '' ?>">

        <h2 class="question">هل تتناول الخضار أو الفواكه يومياً؟</h2>

        <!-- Yes / No Options in Row -->
        <div class="options-row">
          <!-- نعم -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['bmi-check-bp.php']) && $_SESSION['survey_responses']['bmi-check-bp.php'] == 'yes') ? 'on' : '' ?>" onclick="selectSingle(this, 'yes')" id="opt-yes">
            <span class="opt-check"></span>
            <span class="opt-label">نعم</span>
          </button>

          <!-- لا -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['bmi-check-bp.php']) && $_SESSION['survey_responses']['bmi-check-bp.php'] == 'no') ? 'on' : '' ?>" onclick="selectSingle(this, 'no')" id="opt-no">
            <span class="opt-check"></span>
