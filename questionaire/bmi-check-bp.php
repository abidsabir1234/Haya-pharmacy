...
<head>
  ...
  <title>استبيان فحص ضغط الدم - الوزن</title>
  <style>
    @font-face {
...
<div class="page">
  ...
  <div class="card" role="main">
    <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; height: 100%;">
      <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['bmi-check-bp.php']) ? $_SESSION['survey_responses']['bmi-check-bp.php'] : 'no' ?>">

      <h1 class="title">استبيان فحص ضغط الدم</h1>

      <p class="question">هل وزنك أعلى من المعدل الصحي أو مؤشر كتلة الجسم لديك <span class="age-num">≥ 25</span>؟</p>

      <div class="toggle-row">
        <!-- لا — NO -->
        <button type="button" id="btn-no" class="tog-btn <?= (isset($_SESSION['survey_responses']['bmi-check-bp.php']) && $_SESSION['survey_responses']['bmi-check-bp.php'] == 'no') ? 'on' : ((!isset($_SESSION['survey_responses']['bmi-check-bp.php'])) ? 'on' : 'off') ?>" onclick="pick('no')" aria-label="لا">
          <span class="t-icon" id="icon-no"></span>
          لا
        </button>

        <!-- نعم — YES -->
        <button type="button" id="btn-yes" class="tog-btn <?= (isset($_SESSION['survey_responses']['bmi-check-bp.php']) && $_SESSION['survey_responses']['bmi-check-bp.php'] == 'yes') ? 'on' : 'off' ?>" onclick="pick('yes')" aria-label="نعم">
