...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Numbers inside options — use system font so digits render correctly */
    .num {
    ...
    /* 3-option grid: 2 cols top row, 1 col bottom row */
    .options-grid {
      display: grid;
    ...
    /* RTL column placement:
       col 1 = physical LEFT, col 2 = physical RIGHT
       In RTL auto-flow: item1 → col2(right), item2 → col1(left) */
    .opt-btn.opt-right { grid-column: 2; grid-row: 1; }   /* أقل من 25  — RIGHT */
    .opt-btn.opt-left  { grid-column: 1; grid-row: 1; }   /* من 25-30   — LEFT  */
    .opt-btn.opt-third { grid-column: 1; grid-row: 2; }   /* أكثر من 30 — LEFT  */
  ...
  <div class="card" role="main">
    <div class="card-body">
      <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; flex: 1;">
        <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['age-40plus.php']) ? $_SESSION['survey_responses']['age-40plus.php'] : '' ?>">

        <!-- Subheading -->
        <h2 class="sub-heading">مؤشر كتلة الجسم (BMI)</h2>

        <!-- 3 Options -->
        <div class="options-grid">
          <!-- Option 1: أقل من 25 — RIGHT side in RTL (col 2) -->
          <button type="button" class="opt-btn opt-right <?= (isset($_SESSION['survey_responses']['age-40plus.php']) && $_SESSION['survey_responses']['age-40plus.php'] == 'less25') ? 'on' : '' ?>" onclick="selectSingle(this, 'less25')" id="opt-less25">
            <span class="opt-check"></span>
            <span class="opt-label">أقل من <span class="num">25</span> كجم/م²</span>
          </button>

          <!-- Option 2: من 25 إلى 30 — LEFT side in RTL (col 1) -->
          <button type="button" class="opt-btn opt-left <?= (isset($_SESSION['survey_responses']['age-40plus.php']) && $_SESSION['survey_responses']['age-40plus.php'] == '25to30') ? 'on' : '' ?>" onclick="selectSingle(this, '25to30')" id="opt-25to30">
            <span class="opt-check"></span>
            <span class="opt-label">من <span class="num">25</span> إلى <span class="num">30</span> كجم/م²</span>
          </button>

          <!-- Option 3: أكثر من 30 — forced to LEFT column (col 1), row 2 -->
          <button type="button" class="opt-btn opt-third <?= (isset($_SESSION['survey_responses']['age-40plus.php']) && $_SESSION['survey_responses']['age-40plus.php'] == 'more30') ? 'on' : '' ?>" onclick="selectSingle(this, 'more30')" id="opt-more30">
            <span class="opt-check"></span>
            <span class="opt-label">أكثر من <span class="num">30</span> كجم/م²</span>
          </button>
        </div>
