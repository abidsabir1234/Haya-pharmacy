  ...
  <div class="card" role="main">
    <div class="card-body">
      <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; flex: 1;">
        <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['smoking-shisha.php']) ? $_SESSION['survey_responses']['smoking-shisha.php'] : '' ?>">

        <h2 class="question">هل سبق أن ظهر لديك ارتفاع في سكر الدم (على سبيل المثال خلال فحص طبي، أثناء مرض، أو خلال الحمل)؟</h2>

        <!-- Yes / No Options in Row -->
        <div class="options-row">
          <!-- نعم -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['smoking-shisha.php']) && $_SESSION['survey_responses']['smoking-shisha.php'] == 'yes') ? 'on' : '' ?>" onclick="selectSingle(this, 'yes')" id="opt-yes">
            <span class="opt-check"></span>
            <span class="opt-label">نعم</span>
          </button>

          <!-- لا -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['smoking-shisha.php']) && $_SESSION['survey_responses']['smoking-shisha.php'] == 'no') ? 'on' : '' ?>" onclick="selectSingle(this, 'no')" id="opt-no">
            <span class="opt-check"></span>
