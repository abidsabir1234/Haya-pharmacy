...
<div class="page">
  ...
  <div class="card" role="main">
    <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; height: 100%;">
      <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['vitamin-diet.php']) ? $_SESSION['survey_responses']['vitamin-diet.php'] : 'no' ?>">
      ...
      <!-- Question -->
      <p class="question">
        هل نادراً ما تتناول منتجات الألبان أو اللحوم أو الأغذية المدعمة بالفيتامينات؟
      </p>

      <div class="toggle-row">
        <!-- لا — NO -->
        <button type="button" id="btn-no" class="tog-btn <?= (isset($_SESSION['survey_responses']['vitamin-diet.php']) && $_SESSION['survey_responses']['vitamin-diet.php'] == 'no') ? 'on' : ((!isset($_SESSION['survey_responses']['vitamin-diet.php'])) ? 'on' : 'off') ?>" onclick="pick('no')" aria-label="لا">
          <span class="t-icon" id="icon-no"></span>
          لا
        </button>

        <!-- نعم — YES -->
        <button type="button" id="btn-yes" class="tog-btn <?= (isset($_SESSION['survey_responses']['vitamin-diet.php']) && $_SESSION['survey_responses']['vitamin-diet.php'] == 'yes') ? 'on' : 'off' ?>" onclick="pick('yes')" aria-label="نعم">
