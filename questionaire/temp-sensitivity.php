...
<head>
  ...
  <meta name="description" content="استبيان طبي لتقييم أعراض نقص الفيتامينات والمعادن - حياء فارماسي" />
  <style>
    @font-face {
...
<div class="page">
  ...
  <div class="upper-header">
    <div class="header-title-box">
      <img src="../assets/images/arrow.svg" class="header-icon-svg" alt="" />
      <h1 class="header-title">استبيان فحص نقص الفيتامينات والمعادن</h1>
    </div>
  </div>
  ...
  <div class="card" role="main">
    <div class="card-body">
      <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; flex: 1;">
        <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['temp-sensitivity.php']) ? $_SESSION['survey_responses']['temp-sensitivity.php'] : '' ?>">

        <h2 class="question">هل نادراً ما تتناول منتجات الألبان أو اللحوم أو الأغذية المدعمة بالفيتامينات؟</h2>

        <!-- Yes / No Options -->
        <div class="options-grid">
          <!-- نعم -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['temp-sensitivity.php']) && $_SESSION['survey_responses']['temp-sensitivity.php'] == 'yes') ? 'on' : '' ?>" onclick="selectSingle(this, 'yes')" id="opt-yes">
            <span class="opt-check"></span>
            <span class="opt-label">نعم</span>
          </button>

          <!-- لا -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['temp-sensitivity.php']) && $_SESSION['survey_responses']['temp-sensitivity.php'] == 'no') ? 'on' : '' ?>" onclick="selectSingle(this, 'no')" id="opt-no">
            <span class="opt-check"></span>
