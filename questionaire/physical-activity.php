...
<head>
  ...
  <title>استبيان تقييم خطر الإصابة بالسكري النوع الثاني - النشاط البدني</title>
  <meta name="description" content="استبيان طبي لتقييم خطر الإصابة بالسكري النوع الثاني - النشاط البدني" />
  <style>
    @font-face {
...
<div class="page">
  ...
  <div class="card" role="main">
    <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; height: 100%;">
      <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['physical-activity.php']) ? $_SESSION['survey_responses']['physical-activity.php'] : 'no' ?>">
      ...
      <!-- Question -->
      <p class="question">
        هل تمارس عادةً نشاطاً بدنياً لمدة لا تقل عن <span class="age-num">30</span> دقيقة يوميًا أثناء العمل و/أو خلال وقت الفراغ (بما في ذلك الأنشطة اليومية المعتادة)؟
      </p>

      <!-- YES / NO Buttons -->
      <div class="toggle-row">
        <!-- لا — NO -->
        <button type="button" id="btn-no" class="tog-btn <?= (isset($_SESSION['survey_responses']['physical-activity.php']) && $_SESSION['survey_responses']['physical-activity.php'] == 'no') ? 'on' : ((!isset($_SESSION['survey_responses']['physical-activity.php'])) ? 'on' : 'off') ?>" onclick="pick('no')" aria-label="لا">
          <span class="t-icon" id="icon-no"></span>
          لا
        </button>

        <!-- نعم — YES -->
        <button type="button" id="btn-yes" class="tog-btn <?= (isset($_SESSION['survey_responses']['physical-activity.php']) && $_SESSION['survey_responses']['physical-activity.php'] == 'yes') ? 'on' : 'off' ?>" onclick="pick('yes')" aria-label="نعم">
