  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .options-stack {
      display: flex;
      flex-direction: column;
      gap: 8px; /* Reduced gap between options */
      width: 100%;
      max-width: 960px;
      margin-bottom: 15px;
    }
    ...
    .opt-btn {
      ...
      border-radius: 12px;
      font-family: inherit;
      font-size: 14px; /* Smaller text as requested */
      font-weight: 700;
      cursor: pointer;
      background-color: transparent;
      color: #015645;
      border: none; /* BOXES REMOVED as requested */
      transition: all 0.2s;
      width: 100%;
      outline: none;
      text-align: right;
    ...
    /* Simple style: No background color AND no border on 'on' state */
    .opt-btn.on {
      background-color: transparent;
      color: #015645;
      border: none;
    ...
    /* When selected, checkmark appeared inside white box, button stays transparent */
  ...
  <div class="card" role="main">
    <div class="card-body">
      <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; flex: 1;">
        <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['diabetes-cholesterol.php']) ? $_SESSION['survey_responses']['diabetes-cholesterol.php'] : '' ?>">

        <h2 class="question">هل لدى أحد من أقاربك المقربين مرض السكري؟</h2>

        <!-- Options Stack -->
        <div class="options-stack">
          <!-- لا -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['diabetes-cholesterol.php']) && $_SESSION['survey_responses']['diabetes-cholesterol.php'] == 'no') ? 'on' : '' ?>" onclick="selectSingle(this, 'no')" id="opt-no">
            <span class="opt-check"></span>
            <span class="opt-label">لا</span>
          </button>

          <!-- نعم (درجة ثانية) -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['diabetes-cholesterol.php']) && $_SESSION['survey_responses']['diabetes-cholesterol.php'] == 'second_degree') ? 'on' : '' ?>" onclick="selectSingle(this, 'second_degree')" id="opt-second_degree">
            <span class="opt-check"></span>
            <span class="opt-label">نعم، الجد/الجدة أو العم/العمة أو الخال/الخالة أو ابن/ابنة العم/العمة أو ابن/ابنة الخال/الخالة(ولكن ليس أحد الوالدين أو الأخ/الأخت أو الطفل/الابن/الابنة)</span>
          </button>

          <!-- نعم (درجة أولى) -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['diabetes-cholesterol.php']) && $_SESSION['survey_responses']['diabetes-cholesterol.php'] == 'first_degree') ? 'on' : '' ?>" onclick="selectSingle(this, 'first_degree')" id="opt-first_degree">
            <span class="opt-check"></span>
            <span class="opt-label">نعم، أحد الوالدين أو الأخ/الأخت أو الطفل/الابن/الابنة</span>
          </button>
        </div>
