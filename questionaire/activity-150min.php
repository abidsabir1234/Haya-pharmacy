  ...
  <div class="card" role="main">
    <div class="card-body">
      <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; flex: 1;">
        <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['activity-150min.php']) ? $_SESSION['survey_responses']['activity-150min.php'] : '' ?>">

        <h2 class="question">هل تمارس عادةً نشاطًا بدنيًا لمدة لا تقل عن <span class="num">30</span> دقيقة يوميًا أثناء العمل و/أو خلال وقت الفراغ (بما في ذلك الأنشطة اليومية المعتادة)؟</h2>

        <!-- Yes / No Options in Row -->
        <div class="options-row">
          <!-- نعم (on the right in RTL grid or standard layout) -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['activity-150min.php']) && $_SESSION['survey_responses']['activity-150min.php'] == 'yes') ? 'on' : '' ?>" onclick="selectSingle(this, 'yes')" id="opt-yes">
            <span class="opt-check"></span>
            <span class="opt-label">نعم</span>
          </button>

          <!-- لا (on the left) -->
          <button type="button" class="opt-btn <?= (isset($_SESSION['survey_responses']['activity-150min.php']) && $_SESSION['survey_responses']['activity-150min.php'] == 'no') ? 'on' : '' ?>" onclick="selectSingle(this, 'no')" id="opt-no">
            <span class="opt-check"></span>
