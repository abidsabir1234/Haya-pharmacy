...
<div class="page">
  ...
  <div class="card" role="main">
    <form id="surveyForm" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center; height: 100%;">
      <input type="hidden" name="survey_answer" id="survey_answer" value="<?= isset($_SESSION['survey_responses']['fatigue.php']) ? $_SESSION['survey_responses']['fatigue.php'] : 'no' ?>">
      ...
      <!-- Question -->
      <p class="question">
        هل تشعر غالبًا بالتعب أو النعاس، حتى بعد الحصول على قسط كافٍ من الراحة؟
      </p>

      <!-- YES / NO Buttons -->
      <div class="toggle-row">
        <!-- لا — NO -->
        <button type="button" id="btn-no" class="tog-btn <?= (isset($_SESSION['survey_responses']['fatigue.php']) && $_SESSION['survey_responses']['fatigue.php'] == 'no') ? 'on' : ((!isset($_SESSION['survey_responses']['fatigue.php'])) ? 'on' : 'off') ?>" onclick="pick('no')" aria-label="لا">
          <span class="t-icon" id="icon-no"></span>
          لا
        </button>

        <!-- نعم — YES -->
        <button type="button" id="btn-yes" class="tog-btn <?= (isset($_SESSION['survey_responses']['fatigue.php']) && $_SESSION['survey_responses']['fatigue.php'] == 'yes') ? 'on' : 'off' ?>" onclick="pick('yes')" aria-label="نعم">
